    ...
    describe('GET /api/users/:id', function () {
        it('Deve retornar um Json com apenas um usuário', function (done) {
            helpers_1.request(helpers_1.app)
                ...
                .end(function (error, res) {
                helpers_1.expect(res.status).to.equal(HTTPStatus.OK);
                helpers_1.expect(res.body.payload.id).to.be.eq(userDefault.id);
                helpers_1.expect(res.body.payload.name).to.be.eq(userDefault.name);
                helpers_1.expect(res.body.payload.email).to.be.eq(userDefault.email);
    ...
    describe('POST /api/users/create', function () {
        it('Deve criar um novo usuário', function (done) {
            ...
            helpers_1.request(helpers_1.app)
                ...
                .end(function (error, res) {
                helpers_1.expect(res.status).to.equal(HTTPStatus.OK);
                helpers_1.expect(res.body.payload.id).to.be.eq(user.id);
                helpers_1.expect(res.body.payload.name).to.be.eq(user.name);
                helpers_1.expect(res.body.payload.email).to.be.eq(user.email);
                done(error);
            });
        });
    });
    describe('PUT /api/users/:id/update', function () {
        it('Deve atualizar um usuário', function (done) {
            ...
            helpers_1.request(helpers_1.app)
                ...
                .end(function (error, res) {
                helpers_1.expect(res.status).to.equal(HTTPStatus.OK);
                helpers_1.expect(res.body.payload.name).to.be.eq(userUpdate.name);
                helpers_1.expect(res.body.payload.email).to.be.eq(userUpdate.email);
                done(error);
            });
        });
    });
    describe('DELETE /api/users/:id/destroy', function () {
        it('Deve deletar um usuários', function (done) {
            helpers_1.request(helpers_1.app)
